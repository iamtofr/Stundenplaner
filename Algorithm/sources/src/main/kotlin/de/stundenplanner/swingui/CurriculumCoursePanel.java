...
package de.stundenplanner.swingui;
...
public class CurriculumCoursePanel extends SolutionPanel<CourseSchedule> {
    ...
    public static final String LOGO_PATH = "/de/stundenplanner/swingui/curriculumCourseLogo.png";
        ...
        for (Day day : courseSchedule.getDayList()) {
            ...
            for (Period period : day.getPeriodList()) {
                roomsPanel.addRowHeader(HEADER_COLUMN, period,
                        createTableHeader(new JLabel(period.getTimeSlot().toString())));
    ...
    private void fillLectureCells(CourseSchedule courseSchedule) {
        ...
        for (Lecture lecture : courseSchedule.getLectures()) {
            ...
            teachersPanel.addCell(lecture.getCourse().getTeacher(), lecture.getPeriod(),
                    createButton(lecture, color, toolTip));
            for (Curriculum curriculum : lecture.getCourse().getCurriculumList()) {
